...
const Loader = ({ className, style, full, space }: LoaderProps) => {
    const fullClass = full ? 'absolute inset-0 flex items-center justify-center' : '';
    const spacePixel = space ? space : '124px';
    const fullStyle = { backdropFilter: 'blur(50px)', opacity: 0.9, height: `calc(100vh - ${spacePixel})` };
    return (
