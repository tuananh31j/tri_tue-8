...
import { Modal, Button, Descriptions, Table, Tag, Divider, Card, Row, Col, Statistic, Alert, Spin, Input } from 'antd';
import { PrinterOutlined, CheckCircleOutlined, CloseCircleOutlined, ClockCircleOutlined, RobotOutlined } from '@ant-design/icons';
...
const StudentReport = ({ open, onClose, student, sessions }: StudentReportProps) => {
    ...
    const handleGenerateComment = async () => {
        ...
        try {
            ...
            const recentSessions = studentSessions.slice(0, 10).map(session => {
                const studentRecord = session['Điểm danh']?.find(r => r['Student ID'] === student.id);
                const completed = studentRecord?.['Bài tập hoàn thành'];
                ...
                return {
                    ...
                    homework: completed !== undefined && total ? `${completed}/${total}` : undefined,
            ...
            try {
                ...
                console.log('✅ Comment is empty?', !comment || comment.trim().length === 0);
                ...
                if (apiError?.message?.includes('quota') || apiError?.message?.includes('giới hạn')) {
            ...
            setCommentError(error?.message || 'Không thể tạo nhận xét. Vui lòng thử lại sau.');
        } finally {
            setIsGenerating(false);
        }
    };
...
const handlePrint = () => {
                    ...
                    <table>
                        ...
                        <tbody>
                            ${studentSessions.map(s => {
                                ...
                                const completed = record?.['Bài tập hoàn thành'];
    ...
    return (
        ...
        >
            <div ref={printRef}>
                {/* Header */}
                <div className="header" style={{ textAlign: 'center', marginBottom: 24, borderBottom: '2px solid #1890ff', paddingBottom: 16 }}>
