...
import { LeftOutlined, RightOutlined, CalendarOutlined, ClockCircleOutlined, BookOutlined, EnvironmentOutlined } from '@ant-design/icons';
...
const TeacherSchedule = () => {
    ...
    const teacherId = teacherData?.id || userProfile?.teacherId || userProfile?.uid || '';
    ...
    if (myClasses.length === 0) return <div style={{ padding: '24px' }}><Empty description="Bạn chưa được phân công lớp học nào" /></div>;
                                            ...
                                            {events.length === 0 ? (
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                    {events.map((event, idx) => (
                                                        <div
                                                            key={idx}
                                                            onClick={() => navigate(`/workspace/attendance/session/${event.class.id}`, {
                                                                state: { classData: event.class, date: event.date }
                                                            })}
                                                            style={{
                                                                padding: '8px',
