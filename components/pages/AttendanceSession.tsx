...
import { Card, Table, Button, Checkbox, Input, InputNumber, Form, Space, message, Steps, Modal, Tag } from 'antd';
...
const AttendanceSessionPage = () => {
    ...
    const sessionDate: string = location.state?.date || dayjs().format('YYYY-MM-DD');
    ...
    const handleAbsentWithPermissionChange = (studentId: string, withPermission: boolean) => {
        setAttendanceRecords(prev => prev.map(record => {
            if (record['Student ID'] === studentId) {
                const updated = { ...record };
                if (withPermission) {
    ...
    const handleExercisesCompletedChange = (studentId: string, count: number | null) => {
        setAttendanceRecords(prev => prev.map(record => {
            if (record['Student ID'] === studentId) {
                const updated = { ...record };
    ...
    const handleSaveAttendance = () => {
        ...
        const attendancePerson = userProfile?.displayName || userProfile?.email || '';
    ...
    const handleCompleteSession = async () => {
        ...
        try {
            ...
            const completionPerson = userProfile?.displayName || userProfile?.email || '';
            ...
            if (sessionId && existingSession) {
                ...
                const updateData = {
                    ...
                    'Bài tập': homeworkDescription || totalExercises ? {
            ...
            render: (_: any, record: Student) => {
                ...
                return (
                    <Checkbox
                        ...
                        onChange={(e) => handleAbsentWithPermissionChange(record.id, e.target.checked)}
                    />
                );
            }
        },
            ...
            render: (_: any, record: Student) => {
                ...
                return (
                    <InputNumber
                        ...
                        onChange={(value) => handleExercisesCompletedChange(record.id, value)}
                        style={{ width: '100%' }}
                    />
                );
            }
    ...
    return (
        <div style={{ padding: '24px' }}>
            {existingSession && (
                ...
                >
                    <p style={{ margin: 0 }}>
                        ℹ️ Buổi học này đã được điểm danh trước đó. Bạn có thể xem lại hoặc chỉnh sửa thông tin.
                    </p>
                </Card>
            )}
