...
import { Card, Table, Tag, Button, Modal, Descriptions, Space, Input, InputNumber, Checkbox, Popconfirm } from 'antd';
...
const ClassSessionHistory = () => {
    ...
    const [editingHomework, setEditingHomework] = useState({ description: '', total: 0 });
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            const updateData = {
                ...
                'Bài tập': editingHomework.description || editingHomework.total ? {
                    'Mô tả': editingHomework.description,
                    'Tổng số bài': editingHomework.total,
                    'Người giao': selectedSession['Bài tập']?.['Người giao'] || '',
                    'Thời gian giao': selectedSession['Bài tập']?.['Thời gian giao'] || new Date().toISOString()
                } : undefined
            };

            const cleanedData = cleanData(updateData);
            const sessionRef = ref(database, `datasheet/Điểm_danh_sessions/${selectedSession.id}`);
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'present',
            render: (_: any, record: AttendanceSession) => {
                const presentCount = record['Điểm danh']?.filter(r => r['Có mặt']).length || 0;
                const total = record['Điểm danh']?.length || 0;
                return <Tag color="green">{presentCount}/{total}</Tag>;
            },
            width: 100
        },
        {
        ...
        },
        {
            ...
            render: (_: any, record: AttendanceRecord) => (
                <Checkbox
                    ...
                    onChange={(e) => handleRecordChange(record['Student ID'], 'Có mặt', e.target.checked)}
                />
            )
        },
        {
            ...
            render: (_: any, record: AttendanceRecord) => (
                <Checkbox
                    ...
                    onChange={(e) => handleRecordChange(record['Student ID'], 'Đi muộn', e.target.checked)}
        ...
        },
        {
            ...
            render: (_: any, record: AttendanceRecord) => (
                <Checkbox
                    ...
                    onChange={(e) => handleRecordChange(record['Student ID'], 'Vắng có phép', e.target.checked)}
                    disabled={record['Có mặt']}
                />
            )
        },
        {
            ...
            render: (_: any, record: AttendanceRecord) => (
                <InputNumber
                    ...
                    onChange={(value) => handleRecordChange(record['Student ID'], 'Bài tập hoàn thành', value)}
                    disabled={!record['Có mặt']}
                    style={{ width: '100%' }}
                />
            )
        },
        {
            ...
            render: (_: any, record: AttendanceRecord) => (
                <InputNumber
                    ...
                    onChange={(value) => handleRecordChange(record['Student ID'], 'Điểm', value)}
                    disabled={!record['Có mặt']}
                    style={{ width: '100%' }}
                />
            )
        },
        {
            ...
            render: (_: any, record: AttendanceRecord) => (
                <Input
                    ...
                    onChange={(e) => handleRecordChange(record['Student ID'], 'Ghi chú', e.target.value)}
                />
            )
        }
    ];
    ...
    const getStatusTags = (record: AttendanceRecord) => {
        const tags = [];
        if (record['Có mặt']) {
            tags.push(<Tag key="present" color="green">Có mặt</Tag>);
            if (record['Đi muộn']) {
                tags.push(<Tag key="late" color="orange">Đi muộn</Tag>);
            }
        } else {
            tags.push(<Tag key="absent" color="red">Vắng</Tag>);
            if (record['Vắng có phép']) {
                tags.push(<Tag key="permission" color="blue">Có phép</Tag>);
            }
        }
        return tags;
    };
                ...
                {selectedSession && (
                    <div>
                        <Descriptions column={2} bordered style={{ marginBottom: 16 }}>
                            <Descriptions.Item label="Lớp học">{selectedSession['Tên lớp']}</Descriptions.Item>
                            <Descriptions.Item label="Ngày">{dayjs(selectedSession['Ngày']).format('DD/MM/YYYY')}</Descriptions.Item>
                            <Descriptions.Item label="Giờ học">
                                {selectedSession['Giờ bắt đầu']} - {selectedSession['Giờ kết thúc']}
                            </Descriptions.Item>
                            <Descriptions.Item label="Giáo viên">{selectedSession['Giáo viên']}</Descriptions.Item>
                            {selectedSession['Thời gian điểm danh'] && (
                                <Descriptions.Item label="Thời gian điểm danh" span={2}>
                                    {dayjs(selectedSession['Thời gian điểm danh']).format('DD/MM/YYYY HH:mm:ss')}
                                    {selectedSession['Người điểm danh'] && ` - ${selectedSession['Người điểm danh']}`}
                                </Descriptions.Item>
                            )}
                            {selectedSession['Thời gian hoàn thành'] && (
                                <Descriptions.Item label="Thời gian hoàn thành" span={2}>
                                    {dayjs(selectedSession['Thời gian hoàn thành']).format('DD/MM/YYYY HH:mm:ss')}
                                    {selectedSession['Người hoàn thành'] && ` - ${selectedSession['Người hoàn thành']}`}
                                </Descriptions.Item>
                            )}
                        </Descriptions>
                        ...
                        {selectedSession['Bài tập'] && (
                            <Card title="Bài tập" size="small" style={{ marginBottom: 16 }}>
                                <p><strong>Mô tả:</strong> {selectedSession['Bài tập']['Mô tả']}</p>
                                <p><strong>Tổng số bài:</strong> {selectedSession['Bài tập']['Tổng số bài']}</p>
                            </Card>
                        )}

                        <Table
                            columns={[
                                { title: 'STT', key: 'index', width: 60, render: (_: any, __: any, i: number) => i + 1 },
                                { title: 'Tên học sinh', dataIndex: 'Tên học sinh', key: 'name' },
                                { 
                                    title: 'Trạng thái', 
                                    key: 'status', 
                                    render: (_: any, record: AttendanceRecord) => getStatusTags(record)
                                },
                                { title: 'Bài tập', dataIndex: 'Bài tập hoàn thành', key: 'exercises', render: (v: any) => v ?? '-' },
                                { title: 'Điểm', dataIndex: 'Điểm', key: 'score', render: (v: any) => v ?? '-' },
                                { title: 'Ghi chú', dataIndex: 'Ghi chú', key: 'note', render: (v: any) => v || '-' }
                            ]}
                            ...
                            <Space direction="vertical" style={{ width: '100%' }}>
                                <div>
                                    ...
                                    <Input.TextArea
                                        ...
                                        onChange={(e) => setEditingHomework(prev => ({ ...prev, description: e.target.value }))}
                                    />
                                </div>
                                <div>
                                    ...
                                    <InputNumber
                                        ...
                                        onChange={(value) => setEditingHomework(prev => ({ ...prev, total: value || 0 }))}
                                        style={{ width: 200 }}
                                    />
                                </div>
                            </Space>
