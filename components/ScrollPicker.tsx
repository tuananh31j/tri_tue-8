...
const ScrollPicker: React.FC<ScrollPickerProps> = ({ items, defaultIndex = 0, onUpdate }) => {
    ...
    const updatePosition = useCallback((animate: boolean) => {
        if (!scrollRef.current) return;
        const centerOffset = 95; // Adjusted to center the active item perfectly
        const translateY = centerOffset - ((currentIndex + VISIBLE_ITEMS_OFFSET) * ITEM_HEIGHT);

        // Use transform3d for hardware acceleration
        scrollRef.current.style.transition = animate ? 'transform 0.2s cubic-bezier(0.4, 0, 0.2, 1)' : 'none';
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
        ...
        if (now - lastUpdateTime.current > 50) {
            ...
            if (itemsDelta !== 0) {
                const newIndex = Math.max(0, Math.min(items.length - 1, currentIndex + itemsDelta));
    ...
    const handleTouchEnd = (e: React.TouchEvent) => {
        ...
        if (Math.abs(momentum) > 0) {
            const newIndex = Math.max(0, Math.min(items.length - 1, currentIndex + momentum));
            setCurrentIndex(newIndex);
        }
    ...
    return (
        ...
        >
            <div className="absolute left-1 right-1 h-[50px] bg-black/5 border-t border-b border-line rounded-lg pointer-events-none z-10" style={{ top: 'calc(50% - 25px + 3px)' }}></div>
            <div ref={scrollRef} className="absolute w-full left-0 right-0">
                {Array.from({ length: VISIBLE_ITEMS_OFFSET }).map((_, i) => <div key={`pad-start-${i}`} className="h-[50px] w-full"></div>)}
                ...
                {Array.from({ length: VISIBLE_ITEMS_OFFSET }).map((_, i) => <div key={`pad-end-${i}`} className="h-[50px] w-full"></div>)}
            </div>
        </div>
    );
};
